...
class PlayerNameLabel(QWidget):
    def __init__(self, player_name, color, parent=None, font_scale=0.4, current=True):
        ...
        self.indicator.setStyleSheet(f"background-color: {color}; border: 1px solid black;")
...
class EditedLabel(QLabel):
    ...
    def resizeEvent(self, event):
        ...
        if '\n' in self.full_text:
            ...
            new_text = ''
            for line in lines:
                fm = QFontMetrics(line)
                available_width = self.width()
                elided = fm.elidedText(line, Qt.ElideRight, available_width) 
                new_lines.append(elided)
            for new_line in new_lines:
                new_text += new_line
                if new_lines[len(new_lines)-1] != new_line:
                    new_text += '\n'
