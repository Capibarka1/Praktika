from PySide6.QtWidgets import QWidget, QScrollArea, QFrame, QVBoxLayout, QApplication
...
class GameWindow(QWidget):
    ...
    def update_game_field(self):
        ...
        height_need = vert * self.board_size
        self.scroll_field.setAlignment(Qt.AlignCenter)
        
        if width_need > self.scroll_field.width() or height_need > self.scroll_field.height():
            self.scroll_field.setWidgetResizable(False) 
            self.game_field.setFixedSize(int(width_need), int(height_need))
        else:
